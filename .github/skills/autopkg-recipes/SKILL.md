...
18. **Never use `hdiutil` in scripts** — `hdiutil attach`, `detach`, `eject`, and related subcommands are deprecated in macOS 27. Use `/usr/sbin/diskutil image attach` for mounting and `/usr/sbin/diskutil eject` for ejecting. Drop `hdiutil`-only flags (`-noverify`, `-noautoopen`) as they have no equivalent; `-nobrowse` is still supported
